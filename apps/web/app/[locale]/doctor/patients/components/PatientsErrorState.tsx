...
interface PatientsErrorStateProps {
  onRetry: () => void;
  isRetrying?: boolean;
}

export function PatientsErrorState({
  onRetry,
  isRetrying = false,
}: PatientsErrorStateProps) {
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-rose-200 bg-rose-50/50 p-8 text-center transition-colors dark:border-rose-900/50 dark:bg-rose-950/20">
      ...
      <p className="mt-1.5 max-w-md text-xs text-slate-600 dark:text-slate-400">
        We encountered an issue fetching your patient consultation records. Please check your network connection and try again.
      </p>
