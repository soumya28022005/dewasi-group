      ...
      <div className="p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
              Direct shortcuts to core platform management interfaces.
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {actions.map(({ href, title, desc, icon: Icon, color, badge }) => {
            ...
            }[color as "blue" | "purple" | "cyan" | "amber" | "emerald" | "indigo"];

            return (
              <Link
                key={href}
                href={href}
                className="group flex flex-col justify-between rounded-xl border border-slate-200 bg-slate-50/70 p-4 transition-all hover:border-amber-500 hover:bg-amber-50/30 hover:scale-[1.02] active:scale-95 dark:border-slate-800 dark:bg-slate-800/40 dark:hover:border-amber-500/70 dark:hover:bg-slate-800/80 focus:outline-hidden"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xs font-bold text-slate-900 transition group-hover:text-amber-600 dark:text-slate-100 dark:group-hover:text-amber-400">
                        {title}
                      </h3>
                  ...
                  <ArrowUpRight className="h-4 w-4 shrink-0 text-slate-400 transition group-hover:text-amber-600 dark:text-slate-500" />
                </div>
