...
function ClinicMiniCard({ clinic }: { clinic: PublicClinic }) {
  ...
  return (
    <Link href={`/clinics/${clinic.id}`} className="block h-[350px] w-[240px] shrink-0">
      <GradientBorderCard>
        <div className="relative flex h-full flex-col items-center p-5 text-center">
          <div className="relative mt-2">
            <div className="flex w-[6rem] h-[8rem] overflow-hidden items-center justify-center rounded-2xl bg-gradient-to-br from-[#2563EB] to-[#0F766E] text-3xl font-bold text-white shadow-lg">
            ...
            {clinic.isApproved && (
              <BadgeCheck className="absolute -bottom-1.5 -right-1.5 h-6 w-6 rounded-full bg-white text-[#2563EB] shadow-sm ring-2 ring-white" />
            )}
          </div>
          ...
          <div className="mt-4 flex w-full flex-col items-center">
            ...
            {clinic.specialties && clinic.specialties.length > 0 && (
              <p className="mt-1 flex w-full items-center justify-center gap-1.5 truncate text-xs font-medium text-slate-500">
          ...
          <div className="w-full space-y-3">
            {location && (
              <p className="flex w-full items-center justify-center gap-1 truncate text-xs text-slate-400">
                <MapPin className="h-3.5 w-3.5 shrink-0 text-[#0F766E]" />
                <span className="truncate">{location}</span>
              </p>
            )}

            <div className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-slate-50 py-2.5 text-xs font-bold text-slate-600 transition-all group-hover:bg-[#14B8A6]/10 group-hover:text-[#2563EB]">
              View Clinic
...
function ClinicSkeleton() {
  return (
    <div className="flex h-[350px] w-[240px] shrink-0 flex-col items-center rounded-[26px] border border-slate-100 bg-white p-5 shadow-sm">
      <div className="mt-2 w-[6rem] h-[8rem] animate-pulse rounded-2xl bg-slate-200" />
      <div className="mt-5 h-5 w-3/4 animate-pulse rounded-full bg-slate-200" />
