...
import { Stethoscope, BadgeCheck, Star, Award } from "lucide-react";
...
function getDoctorAvatar(doctor: ExtendedDoctor) {
  return (doctor as any).profilePhoto ?? doctor.user?.avatar ?? null;
}
          ...
          <div className="p-3 sm:p-4 lg:p-4.5 xl:p-5">
            <div className="flex flex-col items-center text-center lg:flex-row lg:items-start lg:text-left">
              ...
              <div className="min-w-0 flex-1 w-full">
                ...
                <div className="mt-2 mb-2 flex items-center justify-center gap-1 lg:justify-start">
...
export default function AvailableDoctorsGrid() {
  const { data, isLoading } = usePublicAvailableDoctors();
  ...
  if (doctors.length === 0) {
    return (
      <div className="mx-3 flex min-h-[300px] flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 px-6 text-center sm:mx-4 dark:border-slate-800 dark:bg-slate-900/50">
        ...
        <p className="mt-1 max-w-sm text-xs leading-5 text-slate-500 dark:text-slate-400">
          We couldn't find any doctors matching the current availability.
        </p>
      </div>
    );
  }
